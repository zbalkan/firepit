...
def test_native_reference_lists_are_canonical(tmpdir):
    ...
    try:
        ...
        process = store._query(
            'SELECT opened_connection_refs FROM "process" WHERE pid = 200'
        ).fetchone()
        assert len(process["opened_connection_refs"]) == 1
...
def test_observation_ref_view_expands_object_refs(tmpdir):
    ...
    try:
        ...
        assert len(rows) == 2
        assert {row["number_observed"] for row in rows} == {2}
    finally:
        store.close()
...
def test_explicit_process_and_network_views(tmpdir):
    ...
    try:
        ...
        network = store._query(
            'SELECT src_ipv4, src_ipv6, dst_ipv4, dst_ipv6 '
            'FROM "stixv_network_traffic"'
        ).fetchone()
        assert network["src_ipv4"] == "192.0.2.10"
        assert network["src_ipv6"] is None
        assert network["dst_ipv4"] is None
        assert network["dst_ipv6"] == "2001:db8::10"
    finally:
        store.close()


def test_lookup_does_not_auto_dereference(tmpdir):
    store = get_storage(str(tmpdir.join("lookup.duckdb")), "hunt")
    try:
        store.cache("q1", _bundle())
        row = next(item for item in store.lookup("process") if item["pid"] == 200)
        assert row["parent_ref"].startswith("process--")
        assert "parent_ref.pid" not in row
        assert "parent_pid" not in row
    finally:
        store.close()
