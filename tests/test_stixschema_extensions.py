            ...
            },
            {
                ...
                "extensions": {
                    "http-request-ext": {
                        "request_method": "get",
                        "request_value": "/",
                        "request_header": {"User-Agent": ["test-agent"]},
                    },
...
def test_native_storage_exposes_standard_extensions_as_struct(tmpdir):
    ...
    try:
        ...
        row = store.connection.execute(
            'SELECT extensions, typeof(extensions) FROM "process"'
        ).fetchone()
        assert row[1].startswith("STRUCT(")
        win = row[0]["windows-process-ext"]
        ...
        http = row[0]["http-request-ext"]
        assert http["request_header"]["User-Agent"] == ["test-agent"]
        assert row[0]["tcp-ext"]["src_flags_hex"] == "00000002"
    finally:
        store.close()
...
def test_unknown_extension_members_survive_in_raw_json(tmpdir):
    ...
    try:
        ...
        row = store.connection.execute(
            "SELECT "
            "json_extract_string(_raw, '$.extensions.\"windows-process-ext\".future_standard_field'), "
